...
int main(int argc, char *argv[]) {
	...
	for(uint i(0);i<max(correctionStep,(uint)kmerSizeCorrection.size());++i){
		c=system((prefixCommand+"Bloocoo"+bloocooversion[i]+" -file "+fileToCorrect+" -kmer-size "+kmerSizeCorrection[i]+" -abundance-min 5 -out reads_corrected"+to_string(i)+".fa >>logs/logBloocoo 2>>logslogBloocoo").c_str());
	...
	for(uint i(0);i<kmerList.size();++i){
		...
		c=system((prefixCommand+"kMILL out.unitigs.fa $(("+kmerSize+"-1)) $(("+kmerSize+"-2))>>logBcalm 2>>logBcalm").c_str());
