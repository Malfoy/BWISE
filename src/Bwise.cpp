...
int main(int argc, char *argv[]) {
	...
	for(;indiceGraph<kmerList.size() and (uint)stoi(kmerList[indiceGraph])<=kMax;++indiceGraph){
		...
		if(indiceGraph==1){
			...
			if((uint)stoi(kmerList[indiceGraph])<kMax){
				c=system((prefixCommand+"numbersFilter paths "+to_string(unitigFilter)+" "+to_string(superReadsCleaning)+" dbg"+to_string(indiceGraph)+".fa   $(("+kmerSize+"))  > cleanedPaths 2>>logs/logBgreat").c_str());
				c=system(("python3 "+prefixCommand+"K2000.py cleanedPaths dbg"+to_string(indiceGraph)+".fa  $(("+kmerSize+")) > compacted_unitigs"+to_string(indiceGraph)+".txt  2>>logs/logK2000").c_str());
	...
	start=system_clock::now();
	//K2000
	c=system(("python3 "+prefixCommand+"K2000.py cleanedPaths dbg"+to_string(indiceGraph)+".fa  $(("+kmerSize+"))  > compacted_unitigs.txt  2>>logs/logK2000").c_str());
